...
export default (
  alignmentLines: Ref<AlignmentLineProps[] | null> | any,
) => {
  ...
  const dragElement = (e: MouseEvent, element: PPTElement, activeElementIdList: string[], elementList: PPTElement[]) => {
    ...
    const elOriginLeft = element.left || 0
    const elOriginTop = element.top || 0
